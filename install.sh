...
# ------------------------------------------------------------------
# Prevent .bashrc “unbound variable” errors in non-interactive shells
# ------------------------------------------------------------------
if ! grep -q '^\[\[ \$- != \*i\* \]\] && return' /root/.bashrc; then
  sed -i '1i# Skip the rest in non-interactive shells\n[[ $- != *i* ]] && return\n' /root/.bashrc
fi
...
# -------------------------------------------
# 6. Clone & launch Drosera-Network dashboard
# -------------------------------------------
